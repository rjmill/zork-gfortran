...
21200	IF(.NOT.EXORBF.OR.EXORCF.OR.(OADV(CANDL).NE.WINNER).OR.
	1	(AND(OFLAG1(CANDL),ONBT).EQ.0)) GO TO 21300
...
60000	IF(PRSA.NE.LOOKW) RETURN		! look?
	...
	IF(AND(OFLAG2(WARNI),TCHBT).NE.0) CALL RSPEAK(869)
	RETURN
C
...
62400	PLOOKF=.FALSE.				! clear window flag.
	...
	OFLAG2(HERE-PRM+PLID1)=AND(OFLAG2(HERE-PRM+PLID1),NOT(OPENBT))
62450	PTOUCF=.TRUE.				! touched now.
C
62500	OFLAG1(SCREW)=AND(OFLAG1(SCREW),NOT(NDSCBT))
	IF((OCAN(SCREW).EQ.PKH1).OR.(OCAN(SCREW).EQ.PKH2))
	1	OFLAG1(SCREW)=OR(OFLAG1(SCREW),NDSCBT)
	OFLAG1(STICK)=AND(OFLAG1(STICK),NOT(NDSCBT))
	IF((OCAN(STICK).EQ.PKH1).OR.(OCAN(STICK).EQ.PKH2))
	1	OFLAG1(STICK)=OR(OFLAG1(STICK),NDSCBT)
	OFLAG1(PKEY)=AND(OFLAG1(PKEY),NOT(NDSCBT))
	IF((OCAN(PKEY).EQ.PKH1).OR.(OCAN(PKEY).EQ.PKH2))
	1	OFLAG1(PKEY)=OR(OFLAG1(PKEY),NDSCBT)
	OFLAG1(KEYS)=AND(OFLAG1(KEYS),NOT(NDSCBT))
	IF((OCAN(KEYS).EQ.PKH1).OR.(OCAN(KEYS).EQ.PKH2))
	1	OFLAG1(KEYS)=OR(OFLAG1(KEYS),NDSCBT)
	IF((OROOM(MAT).NE.PRM).AND.(OROOM(MAT).NE.PALRM)) MATF=.FALSE.
	OFLAG1(MAT)=AND(OFLAG1(MAT),NOT(NDSCBT))
	IF(.NOT.MATF) RETURN
	OFLAG1(MAT)=OR(OFLAG1(MAT),NDSCBT)
...
63000	DO 63100 I=1,OLNT			! loop through objects
	...
	1	(AND(OFLAG1(I),TAKEBT).EQ.0)) GO TO 63100
