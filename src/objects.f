...
55000	IF((PRSA.NE.GIVEW).OR.
	1 (AND(OFLAG2(PRSO),NOCHBT).NE.0)) GO TO 55100	! give?
...
6050	DOMEF=.FALSE.				! not tied in dome.
	...
	OFLAG1(TIMBE)=AND(OFLAG1(TIMBE),NOT(NDSCBT))
	OFLAG1(COFFI)=AND(OFLAG1(COFFI),NOT(NDSCBT))
...
28100	I=215					! unlocked, view frm below.
	...
	RFLAG(MGRAT)=AND(RFLAG(MGRAT),NOT(RLIGHT))	! set light/dark.
...
4300	I=652					! mung message.
	...
	1	(AND(OFLAG2(PRSI),WEAPBT).NE.0)) GO TO 4600
	I=653					! novelty 1.
...
21100	IF((AND(OFLAG1(PRSO),VICTBT).EQ.0).AND.
	1  (AND(OFLAG2(PRSO),VILLBT).EQ.0)) GO TO 21200
...
22000	IF(PRSO.NE.STAMP) GO TO 22100		! stamp?
	IF(PRSA.EQ.TAKEW) OFLAG1(BROCH)=AND(OFLAG1(BROCH),NOT(CONTBT))
	GO TO 10				! do normal take.
C
...
29200	IF(PRSO.NE.TTIE) GO TO 29300		! tied object?
	OFLAG1(TTIE)=AND(OFLAG1(TTIE),NOT(NDSCBT))
	OFLAG1(ROPE)=AND(OFLAG1(ROPE),NOT(NDSCBT))
	OFLAG2(ROPE)=AND(OFLAG2(ROPE),NOT(CLMBBT))
...
36500	RVSND=0					! 5, sand collapses.
	...
	1	OFLAG1(STATU)=AND(OFLAG1(STATU),NOT(VISIBT))
...
39700	IF((PRSA.NE.PUTUNW).OR.((PRSO.NE.BLABE).AND.
	...
	CALL NEWSTA(PRSO,1001,XOR(HERE,1),0,0)	! put in other room.
	RETURN
C
...
40100	IF(PRSA.NE.LOOKIW) GO TO 10		! look in?
	...
	SVFLAG=RFLAG(XOR(HERE,1))		! save room flags from other.
	F=MOVETO(XOR(HERE,1),WINNER)		! go to other room.
	F=RMDESC(3)				! describe it.
	F=MOVETO(XOR(HERE,1),WINNER)		! come back.
	RFLAG(XOR(HERE,1))=SVFLAG		! restore flags.
	RETURN
C
...
41200	CALL RSPEAK(987)			! close lid.
	OFLAG2(PRSO)=AND(OFLAG2(PRSO),NOT(OPENBT))
	RETURN
C
C O73--	Keyholes
C
42000	IF(PRSA.NE.LOOKIW) GO TO 42200		! look in?
	...
	2	LIT(XOR(HERE,1))) I=989		! does work
	CALL RSPEAK(I)
	RETURN
C
...
42400	IF((PRSO.NE.SCREW).AND.(PRSO.NE.KEYS).AND.
	...
	IF(QEMPTY(XOR(PRSI,1))) GO TO 10		! nothing to shove.
	DO 42500 I=1,OLNT
	  IF(OCAN(I).EQ.XOR(PRSI,1)) GO TO 42600	! find obj in keyhole.
42500	CONTINUE
	CALL BUG(67,XOR(PRSI,1))
C
42600	CALL NEWSTA(I,992,XOR(HERE,1),0,0)	! obj falls to floor.
...
44100	CALL RSPEAK(1024)			! start vision.
	OFLAG1(OBJ)=AND(OFLAG1(OBJ),NOT(VISIBT))	! object not visible.
...
100	IF(PRSA.NE.DEADXW) GO TO 200		! dead?
	...
	1	GO TO 125			! is it here?
	  ...
	  J=502
	  IF(.NOT.QEMPTY(I).AND.
	1   ((AND(OFLAG1(I),TRANBT).NE.0).OR.
	2    (AND(OFLAG2(I),OPENBT).NE.0))) CALL PRINCO(I,573,.TRUE.)
125	CONTINUE
C
150	J=500
